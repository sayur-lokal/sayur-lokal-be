...
def create_rating(current_user, data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        order = Order.query.filter_by(
            id=data.get("order_id"), buyer_id=current_user.id
        ).first()
...
def update_rating(
    current_user, rating_id: int, data: Dict[str, Any]
    ...
    try:
        ...
        rating = Rating.query.filter_by(id=rating_id, buyer_id=current_user.id).first()
...
def delete_rating(current_user, rating_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        return {"success": True, "message": "Rating berhasil dihapus"}, 200
